...
class PasswordGenerator:
    def __init__(self):
        ...
        self.label_frame = LabelFrame(
            self.window, text="Enter the number of characters", bg="lightblue")
        ...
        generate_btn = Button(
            self.button_frame, text="Generate Password", bg="lightpink", command=self.generate_random_password)
        generate_btn.grid(row=0, column=0, padx=10)
        # Copy Password Button
        copy_btn = Button(self.button_frame,
                          text="Copy Password", bg="lightpink", command=self.copy_password)
